...
import { getUserInfo } from "@/lib/msalGraph";
...
const ThemeMenu = ({ theme, setTheme }: { theme: string | undefined; setTheme: (value: string) => void }) => (
  <DropdownMenuGroup>
    <DropdownMenuSubMenu>
...
export const DropdownUserProfile = ({ children, align = "start" }: DropdownUserProfileProps) => {
  ...
  const fetchGraphData = async () => {
    ...
    try {
      ...
      setUserEmail(userInfo?.mail || userInfo?.userPrincipalName);
    } catch (error) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild aria-label="User Profile Menu">
        {children}
      </DropdownMenuTrigger>
      <DropdownMenuContent
        align={align}
      ...
      >
        <DropdownMenuLabel>
          {userEmail ? userEmail : <span className="italic text-gray-500">No Email Available</span>}
        </DropdownMenuLabel>
